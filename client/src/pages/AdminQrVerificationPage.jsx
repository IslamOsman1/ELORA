...
export default function AdminQrVerificationPage() {
  ...
  const text = {
    ...
    empty: isArabic ? 'لا توجد مواعيد مفتوحة لهذا المستخدم حالياً' : 'This user has no open appointments right now',
    cameraHint: isArabic ? 'اسمح للمتصفح باستخدام الكاميرا ثم وجّهها نحو QR الخاص بالمستخدم.' : 'Allow camera access in the browser, then point it at the user QR code.',
  ...
  async function startScanner() {
    ...
    try {
      ...
      const controls = await readerRef.current.decodeFromVideoDevice(undefined, videoRef.current, async (result) => {
        if (!result) return;
      ...
      <div className="mx-auto max-w-7xl">
        <div className="mb-8 flex flex-wrap items-center justify-between gap-4">
          <div>
            ...
            <h1 className="mt-3 font-display text-5xl">{isArabic ? 'ELORA Scan Desk' : 'ELORA Scan Desk'}</h1>
        ...
        <div className="grid gap-6 xl:grid-cols-[460px_1fr]">
          <section className="premium-card p-5">
            <div className="flex items-center gap-3 text-[#f2d38d]">
              <Camera size={18} />
              <h2 className="text-2xl font-semibold">{isArabic ? 'مسح الكاميرا' : 'Camera scan'}</h2>
            </div>
            ...
            <div className="mt-4 flex flex-wrap gap-3">
              {!cameraEnabled ? (
                <button className="btn-gold inline-flex items-center gap-2" onClick={startScanner} disabled={cameraLoading}>
                  <Camera size={16} />
                  {cameraLoading ? text.camera : text.camera}
                </button>
              ) : (
            ...
            <form onSubmit={handleManualVerify} className="mt-6 grid gap-3">
              ...
              <button className="btn-gold" disabled={lookupLoading}>{lookupLoading ? text.verify : text.verify}</button>
            </form>
          </section>
          ...
          <section className="premium-card p-5">
            <div className="flex items-center gap-3 text-[#f2d38d]">
              <ShieldCheck size={18} />
              <h2 className="text-2xl font-semibold">{isArabic ? 'نتيجة التحقق' : 'Verification result'}</h2>
            </div>
            ...
            {lookupResult ? (
                      ...
                      <article key={appointment._id} className="rounded-[1.75rem] border border-white/10 bg-white/[0.03] p-5">
                        <div className="flex flex-wrap items-start justify-between gap-4">
                          ...
                          <button
                            ...
                            onClick={() => confirmAttendance(appointment._id)}
                            disabled={appointment.attendanceConfirmed}
                          >
              ...
              <div className="mt-6 grid min-h-[420px] place-items-center rounded-[1.75rem] border border-dashed border-white/10 bg-white/[0.02] p-8 text-center text-white/45">
                <div>
                  ...
                  <p className="mt-4">{isArabic ? 'ابدأ المسح بالكاميرا أو ألصق الرمز لعرض بيانات الموعد.' : 'Start a camera scan or paste the code to display appointment details.'}</p>
                </div>
              </div>
            )}
          </section>
